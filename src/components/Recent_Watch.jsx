      ...
      {watched && watched.length > 0 && (
        <div className="px-6 md:px-10 lg:px-[2rem] ">
          ...
          <div className="grid  grid-cols-2  lg:grid-cols-5 gap-5">
            {watched &&
              watched.map((info, index) => (
                <Card key={index} info={info} delet={true} />
              ))}
          </div>
          <i
            className="fa-solid fa-trash-can relative top-1 left-1 text-[#c56f6f] cursor-pointer  z-50 "
            onClick={() => {
              deletwatch(), sethook(!hook);
            }}
          ></i>
        </div>
      )}
